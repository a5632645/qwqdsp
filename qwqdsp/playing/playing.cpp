...
#include "qwqdsp/filter/linkwitz_riley.hpp"
...
static qwqdsp::filter::LinkwitzRiley8 filter;
...
static void AudioInputCallback(void* _buffer, unsigned int frames) {
    ...
    for (auto& s : buffer) {
        auto [lp, hp] = filter.Tick(dsp.Impluse());
        s.l = lp + hp;
        s.r = s.l;
    }
}
...
int main(void) {
    ...
    cutoff.on_value_change = [](float v) {
        filter.SetCutoff(v, kFs);
    };
    dsf_bound.y += dsf_bound.height;
    cutoff.set_bound(dsf_bound);
    cutoff.set_range(20.0f, 20000.0f, 100.0f, 1000.0f);
