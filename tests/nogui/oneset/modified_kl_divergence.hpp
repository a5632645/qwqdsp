...
// ------------------------------------------------------------
// ModifiedKlDivergenceOnsetDetector
// ------------------------------------------------------------
/**
 * @brief 修正 KL 散度（Modified Kullback-Leibler）onset 检测器。
 *
 * 移植自 ref_repos/aubio/src/spectral/specdesc.c:224-234 的
 * aubio_specdesc_mkl。与 KL 不同，对每 bin 取 log 再求和（不做幅度
 * 加权），对大动态范围变化更敏感：
 *
 *   D(t) = Σ_k log(1 + X_t(k) / (X_{t-1}(k) + 0.1))
 *
 * 常数 0.1 为稳定项（aubio 原值）。NaN 结果置 0。
 * 峰值拾取用 aubio 自适应阈值。
 */
class ModifiedKlDivergenceOnsetDetector {
public:
    ...
    OnsetResult Detect(std::span<const float> input, float sample_rate) {
        ...
        for (size_t i = 0; i < N; ++i) {
            ...
            for (size_t k = 0; k < bins; ++k) {
                ...
                const float term = std::log1p(cur / (prev + 0.1f));
        ...
        picker.SetThreshold(0.05f); // aubio mkl 默认
