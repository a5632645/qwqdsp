...
int main() {
    ...
    constexpr size_t kHopSize = 4;

    // 默认模式: clamp 到最大修正范围
    // 若需跳过无法完全量化的 bin，使用 PitchQuantizer<false>
    ...
    file.save(qwqdsp_support::OutputFile("pitch_quatizer.wav"));

    std::cout << "saved pitch_quatizer.wav\n" << std::flush;
    return 0;
}
