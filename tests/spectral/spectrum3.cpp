...
static void Octave_Init() {
    ...
    int i = 1;
    do {
        fcenter = kCenterFreqFloor * std::exp2(static_cast<float>(i++) / static_cast<float>(kOctave));
        ...
        octave_idx_.push_back(OctaveSmoothIndex{i32_floor_bin, i32_ceil_bin, fcenter});
...
static void InitLanczosTable() {
    ...
    for (int px = 0; px < kCanvasW; ++px) {
        ...
        int end = std::min(octaveCnt - 1, static_cast<int>(std::floor(center + kLanczosA)));
        float wsum = 0.0f;
        for (int j = start; j <= end; ++j) {
            ...
            if (x == 0.0f) {
                ...
                float pix = kPi * x;
                w = static_cast<float>(kLanczosA) * std::sin(pix) * std::sin(pix / static_cast<float>(kLanczosA))
                  / (pix * pix);
            }
    ...
    for (int px = 0; px < kCanvasW; ++px) {
        ...
        if (logF >= octave_log_freq_[octaveCnt - 1]) {
            ...
            for (int j = 0; j < octaveCnt - 1; ++j) {
                if (logF >= octave_log_freq_[j] && logF < octave_log_freq_[j + 1]) {
                    seg = j;
                    break;
                }
            }
        ...
        if (left == mid)
            right = std::min(octaveCnt - 1, mid + 2);
        if (right == mid)
            left = std::max(0, mid - 2);
        mid = left + 1;
...
static void DrawXAxisGrid(int octaveCnt) {
    ...
    for (float start : kDecadeStart) {
        ...
        for (float freq = start; freq <= start * 10.0f; freq += step) {
            ...
            DrawLine(static_cast<int>(sx), kCanvasY, static_cast<int>(sx), kCanvasY + kCanvasH, kGridColor);
        }
    }
    ...
    for (float freq : kXTickLabels) {
        ...
        int tw = MeasureText(label, 10);
        DrawText(label, static_cast<int>(sx) - tw / 2, kCanvasY + kCanvasH + 4, 10, kTextColor);
    }
}
...
static void DrawFFTEnvelope(int octaveCnt) {
    ...
    if (pts.size() > 1)
        DrawLineStrip(pts.data(), static_cast<int>(pts.size()), Color{80, 220, 120, 180}); // 浅绿半透明
}
...
static void DrawOctaveCurve2(int /*octaveCnt*/) {
    ...
    for (int px = 0; px < kCanvasW; ++px) {
        ...
        float dbVal = std::max(tap.w_l * octave_gain_dB_[tap.idx_l] + tap.w_m * octave_gain_dB_[tap.idx_m]
                                   + tap.w_r * octave_gain_dB_[tap.idx_r],
                               kDbFloor);
        float norm = (dbVal - kDbFloor) / (-kDbFloor);
        float y = kCanvasY + kCanvasH - norm * kCanvasH;
        curvePoints[px] = {static_cast<float>(kCanvasX + px), y};
...
static void DrawOctaveScatter(int octaveCnt) {
    ...
    for (int i = 0; i < octaveCnt; ++i) {
        ...
        float sx = kCanvasX + (octave_log_freq_[i] - logMin) / (logMax - logMin) * kCanvasW;
