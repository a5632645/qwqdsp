...
static constexpr uint16_t kSptLen = SPECTRUM_SIZE;    // 256
static constexpr uint8_t kBinNum = SPECTRUM_BIN_SIZE; // 25

static int16_t gPcmBuffer[kSptLen]; // int16 PCM 帧
static float gFloatRing[kSptLen];   // float 环形暂存
static size_t gFloatPos = 0;        // 当前写入位置

static uint32_t gBinOut[kBinNum];         // spectrum_process 输出 (能量)
static uint8_t gLevels[kBinNum];          // 量化电平 (0~126)
...
extern "C" void MaCaptureCallback(ma_device* pDevice, void* pOutput,
    ...
    for (ma_uint32 i = 0; i < frameCount; ++i) {
        ...
        if (gFloatPos >= kSptLen) {
            ...
            // 频谱处理 + 电平量化
            if (spectrum_process(&gSpectrumCtx, gPcmBuffer, gBinOut)) {
                spectrum_quantized_level(&gSpectrumCtx, gBinOut);

                // 将量化电平拷贝到渲染缓冲区 (release 语义保证数据可见)
                std::memcpy(gLevels, gSpectrumCtx.level_dst, kBinNum);
...
static void DrawSpectrumBars() {
    ...
    // ── 水平参考线 ──
    constexpr int kRefTicks[] = { 0, 25, 50, 75, 100, 126 };
    for (int ref : kRefTicks) {
        int y = canvasY + canvasH - (ref * canvasH / SPECTRUM_LEVEL_NUM);
    ...
    for (uint8_t i = 0; i < kBinNum; ++i) {
        ...
        int   lev = gLevels[i];
        int   barH = (lev * canvasH) / SPECTRUM_LEVEL_NUM;
    ...
    // ── 频率标签 (底部) ──
    uint16_t* freqTab = spectrum_get_freq_interval(&gSpectrumCtx);
    for (uint8_t i = 0; i < kBinNum; ++i) {
        float cx = canvasX + i * barTotalW + barTotalW * 0.5f;
        char  label[16];
        if (freqTab[i] >= 1000)
            snprintf(label, sizeof(label), "%.1fk", freqTab[i] / 1000.0f);
        else
            snprintf(label, sizeof(label), "%d", freqTab[i]);
    ...
    for (int ref : kRefTicks) {
        int y   = canvasY + canvasH - (ref * canvasH / SPECTRUM_LEVEL_NUM);
        float db = -80.0f + (ref * 80.0f / SPECTRUM_LEVEL_NUM);
        char lbl[16];
...
int main(void) {
    // ── 初始化频谱驱动 ──
    spectrum_init(&gSpectrumCtx, static_cast<uint16_t>(kSampleRate),
                  kSptLen, kBinNum);
