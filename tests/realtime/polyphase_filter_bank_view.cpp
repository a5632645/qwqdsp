...
public:
    ...
    void Init() {
        ...
        qwqdsp_filter::WindowFIR::Lowpass(
            ir, std::numbers::pi_v<float> / static_cast<float>(kNumSubbands));
        ...
        for (int k = 0; k < kNumSubbands; ++k) {
            for (int j = 0; j < kPolyphaseLen; ++j) {
                ...
                coeffs_[k * kPolyphaseLen + j] =
                    (src < kPrototypeLen) ? ir[src] : 0.0f;
            }
        }
    ...
    void Process(const float* input) {
        ...
        for (int k = 0; k < kDisplayBands; ++k) {
            float mag_sq = fft_real_[k] * fft_real_[k]
                         + fft_imag_[k] * fft_imag_[k];
    ...
    // ── 存取器 ──
    std::span<const float> GetDB()    const noexcept { return smoothed_db_; }
    int                    NumBands() const noexcept { return smoothed_db_.size(); }
...
static void DrawAxes() {
    ...
    for (float start : kDecadeStart) {
        float step = start;
        for (float freq = start;
             freq <= start * 10.0f && freq <= kDisplayFreqMax;
             freq += step) {
            if (freq < kDisplayFreqMin) continue;
    ...
    for (float freq : kXTickLabels) {
        if (freq < kDisplayFreqMin || freq > kDisplayFreqMax) continue;
        float norm = (std::log10(freq) - logFMin) / logFRange;
        int x = kMarginLeft + static_cast<int>(norm * kCanvasWidth);
        const char* lbl = (freq >= 1000.0f)
            ? TextFormat("%.0fk", freq / 1000.0f)
            : TextFormat("%.0f",  freq);
    ...
    // ── 坐标轴 ──
    DrawLine(kMarginLeft, kMarginTop,
             kMarginLeft, kMarginTop + kCanvasHeight, WHITE);
    DrawLine(kMarginLeft, kMarginTop + kCanvasHeight,
             kMarginLeft + kCanvasWidth, kMarginTop + kCanvasHeight, WHITE);
}
...
static void DrawSubbandCurve(std::span<const float> dB) {
    ...
    for (int px = 0; px < kCanvasWidth; ++px) {
        ...
        float dbVal = dB[base]     * d * (d - 1.0f) * 0.5f
                    + dB[base + 1] * (1.0f - d * d)
                    + dB[base + 2] * d * (d + 1.0f) * 0.5f;
...
int main(void) {
    ...
    InitWindow(kWindowWidth, kWindowHeight,
               "Polyphase Analysis Filter Bank — qwqdsp + raylib");
    SetTargetFPS(60);
    ...
    if (audio_ok) {
        ...
        TraceLog(LOG_WARNING,
                 "miniaudio init failed — running in silent mode");
    }
    ...
    while (!WindowShouldClose()) {
        ...
        float peak_freq = static_cast<float>(peak_bin) * kSampleRate
                        / static_cast<float>(kNumSubbands);
        ...
        DrawText(
            TextFormat("M=%d  L=%d  |  "
                       ...
                       kNumSubbands, kPrototypeLen,
                       peak_bin, peak_freq, peak_db,
                       s_cb_us_last, s_cb_us_avg, s_cb_us_max, GetFPS()),
            10, 10, 14, WHITE);
