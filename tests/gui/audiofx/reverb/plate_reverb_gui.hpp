...
// ------------------------------------------------------------
//  PlateReverbGui — 板混响的完整 GUI 包装器
// ------------------------------------------------------------
struct PlateReverbGui {
    // ── 原子参数（UI 线程写入，音频线程读取）──
    ...
    static constexpr const char* kWindowTitle = "Plate Reverb — PlateReverb";
    ...
    void CreateKnobs() {
        ...
        knobs_[3] = MakeKnob(kx, ky, kKnobW, kKnobH, "Size", 0.5f, 2.0f, 0.01f, 1.0f);
        ...
        // ── 旋钮回调绑定 ──
        std::atomic<float>* param_ptrs[kNumKnobs] = {&mix_, &predelay_, &decay_, &size_, &lowpass_, &damping_};
        ...
        // 强制同步初始值
        mix_.store(knobs_[0].get_value(), std::memory_order_relaxed);
        predelay_.store(knobs_[1].get_value(), std::memory_order_relaxed);
        decay_.store(knobs_[2].get_value(), std::memory_order_relaxed);
        size_.store(knobs_[3].get_value(), std::memory_order_relaxed);
        lowpass_.store(knobs_[4].get_value(), std::memory_order_relaxed);
        damping_.store(knobs_[5].get_value(), std::memory_order_relaxed);
    ...
    static constexpr size_t kNumKnobs = 6;
