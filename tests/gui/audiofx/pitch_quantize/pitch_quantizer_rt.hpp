...
public:
    ...
    void Process(std::span<float> block) noexcept {
        ...
        aso_.Process(block, [this](std::span<const float> in, std::span<float> out) {
            ProcessFrame(in, out);
        });
    ...
    void SetColor(float color) noexcept { color_ = std::clamp(color, 0.0f, 1.0f); }
    void SetLowCut(float hz) noexcept { low_cut_hz_ = hz; }
    void SetHighCut(float hz) noexcept { high_cut_hz_ = hz; }
