...
struct PitchQuantizerRTGuiV2 {
    ...
    // 根音名称
    static constexpr const char* kRootNames[12] = {"C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"};
    ...
    void CreateKnobs() {
        ...
        low_cut_knob_.on_value_change = [this](float v) { low_cut_hz_.store(v, std::memory_order_relaxed); };
        ...
        high_cut_knob_.on_value_change = [this](float v) { high_cut_hz_.store(v, std::memory_order_relaxed); };
        ...
        mix_knob_.on_value_change = [this](float v) { mix_.store(v, std::memory_order_relaxed); };
    ...
    void DisplayKnobs() {
        ...
        DrawSelector(Rectangle{selector_root_x_, selector_root_y_, selector_root_w_, 30.0f}, 12, kRootNames,
                     static_cast<size_t>(root_note_.load(std::memory_order_relaxed)), mouse_pos, [this](size_t i) {
        ...
        DrawSelector(Rectangle{selector_root_x_, selector_scale_y_, selector_scale_w_, 30.0f}, kNumScaleTypes,
                     kScaleNames, static_cast<size_t>(scale_type_.load(std::memory_order_relaxed)), mouse_pos,
                     [this](size_t i) {
...
private:
    ...
    static void DrawSelector(Rectangle bound, size_t num_items, const char* const* names, size_t selected,
                             Vector2 mouse_pos, std::function<void(size_t)> on_click) {
