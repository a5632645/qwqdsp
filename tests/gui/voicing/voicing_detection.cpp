...
// 帧分析参数
static constexpr size_t kFrameSize = 2048;   // ~42.7ms (每帧输出一个 V/UV 判决 ≈ 10ms)
...
static float g_threshold = 0.35f;
static float g_unvoiced_gain_db = -6.0f;
static float g_voiced_gain_db = 0.0f;

// 检测器指针 — 主线程借此设置检测器参数
static SubbandVoicingDetector* g_detector_ptr = nullptr;
...
extern "C" void MaDuplexCallback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount) {
    ...
    // 静态实例 (仅音频线程访问), 暴露指针供主线程调参
    static SubbandVoicingDetector s_detector(kSampleRate);
    if (!g_detector_ptr)
        g_detector_ptr = &s_detector;
    static size_t s_frame_count = 0;

    for (ma_uint32 i = 0; i < frameCount; ++i) {
        float sample = src[i];

        s_detector.processSample(sample);
        ++s_frame_count;

        if (s_frame_count == kFrameSize) {
            float energy_ratio, rms_linear;
            float prob = s_detector.frameResult(energy_ratio, rms_linear);

            g_voicing_prob = prob; g_energy_ratio = energy_ratio;
            (void)rms_linear;
        ...
        // ── 输出: 当前增益 × 采样 ──
        float prob = s_detector.lastProbability();
...
static void DrawProbabilityBar(float prob, float threshold) {
    ...
    if (prob >= threshold) {
        ...
        fill_color = {
            ...
            static_cast<unsigned char>(180 + 75 * t),
            static_cast<unsigned char>(60 - 40 * t),
            255
        };
    }
...
int main(void) {
    ...
    InitWindow(kWindowWidth, kWindowHeight,
               "V/UV Detection - Sub-band Energy Ratio  |  miniaudio + raylib");
    SetTargetFPS(60);
    ...
    threshold_knob.set_range(0.0f, 1.0f, 0.01f, 0.35f);
    threshold_knob.set_bound(30, 120, 200, 50);
    threshold_knob.value_to_text_function = [](float v) -> std::string {
        char buf[16]; snprintf(buf, sizeof(buf), "%.2f", v); return buf;
    };
    ...
    lp_scale_knob.on_value_change = [](float v) { if (g_detector_ptr) g_detector_ptr->setLpScale(v); };
    ...
    hp_scale_knob.on_value_change = [](float v) { if (g_detector_ptr) g_detector_ptr->setHpScale(v); };

    // ── Row 2: 滤波器频率 + Delta ──
    Knob lp_freq_knob;
    lp_freq_knob.set_title("LP Freq");
    lp_freq_knob.set_range(100.0f, 3000.0f, 10.0f, 800.0f);
    lp_freq_knob.set_bound(30, 190, 200, 50);
    lp_freq_knob.value_to_text_function = [](float v) -> std::string {
        char buf[16]; snprintf(buf, sizeof(buf), "%.0f Hz", v); return buf;
    };
    lp_freq_knob.on_value_change = [](float v) { if (g_detector_ptr) g_detector_ptr->setLpFreq(v); };

    Knob hp_freq_knob;
    hp_freq_knob.set_title("HP Freq");
    hp_freq_knob.set_range(500.0f, 8000.0f, 10.0f, 3000.0f);
    hp_freq_knob.set_bound(260, 190, 200, 50);
    hp_freq_knob.value_to_text_function = [](float v) -> std::string {
        char buf[16]; snprintf(buf, sizeof(buf), "%.0f Hz", v); return buf;
    };
    hp_freq_knob.on_value_change = [](float v) { if (g_detector_ptr) g_detector_ptr->setHpFreq(v); };
    ...
    delta_knob.value_to_text_function = [](float v) -> std::string {
        char buf[16]; snprintf(buf, sizeof(buf), "%.4f", v); return buf;
    };
    delta_knob.on_value_change = [](float v) { if (g_detector_ptr) g_detector_ptr->setDelta(v); };

    // ── Row 3: 概率映射 + RMS 时间 ──
    Knob center_knob;
    center_knob.set_title("Center");
    center_knob.set_range(-2.0f, 4.0f, 0.05f, 1.0f);
    center_knob.set_bound(30, 260, 200, 50);
    center_knob.value_to_text_function = [](float v) -> std::string {
        char buf[16]; snprintf(buf, sizeof(buf), "%.2f", v); return buf;
    };
    center_knob.on_value_change = [](float v) { if (g_detector_ptr) g_detector_ptr->setCenter(v); };

    Knob width_knob;
    width_knob.set_title("Width");
    width_knob.set_range(0.1f, 5.0f, 0.05f, 2.5f);
    width_knob.set_bound(260, 260, 200, 50);
    width_knob.value_to_text_function = [](float v) -> std::string {
        char buf[16]; snprintf(buf, sizeof(buf), "%.2f", v); return buf;
    };
    width_knob.on_value_change = [](float v) { if (g_detector_ptr) g_detector_ptr->setWidth(v); };

    Knob rms_tau_knob;
    rms_tau_knob.set_title("RMS Tau");
    rms_tau_knob.set_range(0.005f, 0.2f, 0.005f, 0.03f);
    rms_tau_knob.set_bound(490, 260, 200, 50);
    rms_tau_knob.value_to_text_function = [](float v) -> std::string {
        char buf[16]; snprintf(buf, sizeof(buf), "%.0f ms", v * 1000.0f); return buf;
    };
    rms_tau_knob.on_value_change = [](float v) { if (g_detector_ptr) g_detector_ptr->setRmsTau(v); };
    ...
    unvoiced_gain_knob.value_to_text_function = [](float v) -> std::string {
        char buf[16]; snprintf(buf, sizeof(buf), "%.1f dB", v); return buf;
    };
    ...
    while (!WindowShouldClose()) {
        ...
        // 映射曲线
        float center = g_detector_ptr ? g_detector_ptr->center() : 1.0f;
        float width  = g_detector_ptr ? g_detector_ptr->width() : 2.5f;
