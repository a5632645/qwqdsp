...
struct TfDerivativeReassignmentFramePeakFilter {
    ...
    void Process(std::span<const float> raw_frame, std::span<const float> /*window*/,
        ...
        // ── 低频合并边界 ──
        const float kMergeHz = 2.0f * bin_hz * zeroPad_;

        float c_sum_power = 0.0f;
        float c_sum_freq = 0.0f;
        float c_sum_time = 0.0f;
        float c_sum_mag = 0.0f;
        ...
        for (int k = 0; k < binSize_; ++k) {
            ...
            // ── 低频合并: 功率重心叠加 ──
            if (inst_freq_hz < kMergeHz) {
                c_sum_power += mag_sq;
                c_sum_freq += inst_freq_hz * mag_sq;
                c_sum_time += group_delay * mag_sq;
                c_sum_mag += mag_lin;
                continue;
            }
        ...
        // ── 输出低频重心点 ──
        if (c_sum_power > 0.0f) {
            float f_c = c_sum_freq / c_sum_power;
            float gd_c = c_sum_time / c_sum_power;
            if (f_c < freqMin_)
                f_c = freqMin_;

            float logF = std::log10(f_c);
            float norm = (logF - logMin_) / (logMax_ - logMin_);
            float y_pos = static_cast<float>(outputHeight_ - 1) * (1.0f - norm);
            float c_pos = (gd_c + 0.5f) * subColScale_;
            ...
            if (y_last && c_last) {
                col_buf_[c_idx * outputHeight_ + y_idx] += c_sum_mag;
            }
            else if (y_last) {
                float c_frac = c_pos - static_cast<float>(c_idx);
                col_buf_[c_idx * outputHeight_ + y_idx] += c_sum_mag * (1.0f - c_frac);
                col_buf_[(c_idx + 1) * outputHeight_ + y_idx] += c_sum_mag * c_frac;
            }
            else if (c_last) {
                float y_frac = y_pos - static_cast<float>(y_idx);
                col_buf_[c_idx * outputHeight_ + y_idx] += c_sum_mag * (1.0f - y_frac);
                col_buf_[c_idx * outputHeight_ + y_idx + 1] += c_sum_mag * y_frac;
            }
            else {
                float c_frac = c_pos - static_cast<float>(c_idx);
                float y_frac = y_pos - static_cast<float>(y_idx);
                col_buf_[c_idx * outputHeight_ + y_idx] += c_sum_mag * (1.0f - c_frac) * (1.0f - y_frac);
                col_buf_[(c_idx + 1) * outputHeight_ + y_idx] += c_sum_mag * c_frac * (1.0f - y_frac);
                col_buf_[c_idx * outputHeight_ + y_idx + 1] += c_sum_mag * (1.0f - c_frac) * y_frac;
                col_buf_[(c_idx + 1) * outputHeight_ + y_idx + 1] += c_sum_mag * c_frac * y_frac;
            }
        }
