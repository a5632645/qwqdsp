...
// 用法:
//   Pyin pyin;
//   pyin.Init(sample_rate, block_size, step_size);
//
//   for (each frame) {
//       pyin.Process(audio_block);
//   }
//
//   auto pitch_track = pyin.GetPitchTrack();  // 获取 HMM 平滑后的基频轨迹
//
// 参考:
//   Mauch & Dixon, "pYIN: A Fundamental Frequency Estimator
...
public:
        ...
        for (size_t ic = 0; ic < n_candidate; ++ic) {
            ...
            for (size_t f = 0; f < n_frame && f < path.size(); ++f) {
                ...
                if (freq > 0) {
                    ...
                    freq_count[ic]++;
                    prev_freq = static_cast<float>(freq);
                }
                // 无声帧保持 0
            }
        }
    ...
    size_t NumFrames() const noexcept {
        return frame_count_;
    }
private:
    float fs_{};
    ...
    // 每帧的 (midi_pitch, probability) 对列表
