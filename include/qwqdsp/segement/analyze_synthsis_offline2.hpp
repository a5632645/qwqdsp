...
public:
    ...
    void Process(
        ...
        // 处理音频
        {
            ...
            while (!input.IsEnd()) {
                size_t need = input_size_ - input_wpos_;
                auto in = input.GetSome(need);
                std::copy(in.begin(), in.end(), input_buffer_.begin() + input_wpos_);
                input_wpos_ += in.size();
                if (input_wpos_ >= input_size_) {
                    size_t copy_len = std::min(input_buffer_.size(), process_buffer_.size());
                    std::copy_n(input_buffer_.begin(), copy_len, process_buffer_.begin());
                    func(std::span<const float>{input_buffer_.data(), input_size_}, std::span<float>{process_buffer_.data(), output_size_});
                    ...
                    write_end_ = write_add_end_ + output_size_;
                ...
                if (write_add_end_ >= 0) {
                    ...
                    for (int i = 0; i < extractSize; ++i) {
                        output.emplace_back(output_buffer_[i] / ((float)output_size_ / output_hop_));
                    }
    ...
    size_t GetMinOutputSize(size_t input_size) const noexcept {
        ...
        return (num_frame - 1) * output_hop_ + output_size_;
    }
    ...
    void SetInputSize(size_t size) noexcept {
        ...
        size_t process_size = std::max(input_size_, output_size_);
        process_buffer_.resize(process_size);
    }
